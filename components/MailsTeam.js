// app/[locale]/components/MailsTeam.jsx
'use client';
...
const TEAM_CSV_URL = process.env.NEXT_PUBLIC_TEAM_CSV_URL || '';
const TEAM_GAS_URL = process.env.NEXT_PUBLIC_TEAM_GAS_URL || '';

export default function MailsTeam() {
  const t = useTranslations('MailsTeam');
  ...
  const fetchTeam = async () => {
    try {
      ...
      if (!response.ok) throw new Error(t('errors.fetchTeamError'));
      ...
      const filteredTeam = parsed.filter((member) => {
        ...
        return roles.some((role) =>
          ['Revisor', 'Editor de Sección', 'Editor en Jefe'].includes(role)
        );
      });
      const director = parsed.find((member) =>
        member['Rol en la Revista']
          ?.split(';')
          .map((role) => role.trim())
          .includes('Director General')
      );
      setTeam(filteredTeam);
      setDirectorGeneral(director ? director.Nombre : t('defaultDirector'));
      setSelected(filteredTeam.map((member) => member.Correo));
    } catch (err) {
      setStatus(t('errors.genericError', { message: err.message }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendEmail = async () => {
    if (!subject.trim() || !body.trim() || selected.length === 0) {
      setStatus(t('errors.missingFields'));
      return;
    }
    if (!TEAM_GAS_URL) {
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="signature">
                <p>${t('signature')}</p>
                <p class="title">${t('directorTitle')}</p>
                <p>${directorGeneral}</p>
              </div>
            </div>
            <div class="footer">
  ...
  if (loading) return <div className="p-4 text-center">{t('loadingTeam')}</div>;
    ...
    <div className="mt-8">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" onClick={closeModal}>
          ...
          >
            <h3 className="text-lg font-medium text-gray-900 mb-4">{t('modalTitle')}</h3>
