...
const ARTICULOS_GAS_URL = process.env.NEXT_PUBLIC_ARTICULOS_SCRIPT_URL || '';
...
const uploadPDFToGitHub = async (base64Content, fileName, message, sha = null) => {
  const t = useTranslations('DirectorPanel');
  if (!GH_TOKEN) throw new Error(t('errors.noGitHubToken'));
  ...
  if (!response.ok) {
    ...
    throw new Error(`${t('errors.uploadFailed')}: ${response.status} - ${errorText}`);
  }
...
const deletePDFFromGitHub = async (fileName, message) => {
  ...
  if (!getRes.ok) throw new Error(`${t('errors.fetchFileInfoFailed')}: ${getRes.status}`);
  ...
  if (!delRes.ok) throw new Error(`${t('errors.deleteFailed')}: ${delRes.status}`);
};
...
const triggerRebuild = async () => {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`${t('errors.rebuildFailed')}: ${response.status} - ${errorText}`);
  }
};
...
export default function DirectorPanel({ user }) {
  ...
  const fetchArticles = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`${t('errors.fetchCsvFailed')}: ${response.status}`);
      }
      ...
      setStatus(`${t('errors.loadArticlesFailed')}: ${err.message}`);
      setArticles([]);
    } finally {
      setLoading(false);
    }
  ...
  const submitToSheet = async (action, articleData, numero = null) => {
    const t = useTranslations('DirectorPanel');
    if (!ARTICULOS_GAS_URL) {
      throw new Error(t('errors.noGasUrl'));
    }
    ...
    if (resText.includes('error')) {
      throw new Error(`${t('errors.gasError')}: ${resText}`);
    }
  ...
  const handleSubmit = async (action = 'add') => {
    ...
    if (!formData.titulo.trim() || !formData.autores.trim()) {
      setStatus(t('errors.titleAndAuthorsRequired'));
      return;
    }
    ...
    try {
      ...
      if (formData.pdfFile) {
        if (!GH_TOKEN) {
          setStatus(t('metadataSavedUploadManually'));
          return;
        }

        setStatus(t('uploadingPdf'));
        const base64 = await toBase64(formData.pdfFile);
        const fileName = `Articulo${articleNumber}.pdf`;
        const message = `${action === 'add' ? t('addPdfMessage') : t('updatePdfMessage')} ${articleNumber}`;
        ...
        setStatus(t(action === 'add' ? 'articleAddedWithPdf' : 'articleUpdatedWithPdf'));
      } else {
        setStatus(t(action === 'add' ? 'articleAdded' : 'articleUpdated'));
      }
      ...
      if (GH_TOKEN) {
        ...
        setStatus(`${status} ${t('rebuildStarted')}`);
      }
  ...
  const handleDelete = async (numero) => {
    const t = useTranslations('DirectorPanel');
    if (!window.confirm(t('confirmDelete', { numero }))) return;
    ...
    try {
      ...
      if (GH_TOKEN) {
        await deletePDFFromGitHub(`Articulo${numero}.pdf`, `${t('deletePdfMessage')} ${numero}`);
      }
      
      setStatus(t('articleDeleted'));
      await fetchArticles();
  ...
  const handleRebuild = async () => {
    const t = useTranslations('DirectorPanel');
    if (!GH_TOKEN) {
      setStatus(t('errors.noGitHubToken'));
      return;
    }

    try {
      setStatus(t('startingRebuild'));
      await triggerRebuild();
  ...
  const ConfigStatus = () => (
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
      <div className="flex items-center space-x-2">
        ...
        <div className="ml-3">
          <h3 className="text-sm font-medium text-yellow-800">{t('configRequired')}</h3>
          <div className="mt-2 text-sm text-yellow-700 space-y-1">
            <p>
              ✅ {t('gasConfig')}{' '}
              <span className={ARTICULOS_GAS_URL ? 'text-green-600' : 'text-red-600'}>
                {ARTICULOS_GAS_URL ? t('configured') : t('notConfigured')}
              </span>
            </p>
            <p>
              ✅ {t('githubTokenConfig')}{' '}
              <span className={GH_TOKEN ? 'text-green-600' : 'text-red-600'}>
                {GH_TOKEN ? t('configured') : t('notConfigured')}
              </span>
            </p>
          </div>
        </div>
        ...
        {status && (
          <div
            className={`mb-6 p-4 rounded-lg ${
              status.includes(t('errors.error')) || status.includes('❌')
                ? 'bg-red-50 border border-red-200 text-red-800'
                : status.includes(t('processing')) || status.includes(t('uploadingPdf'))
        ...
        {uploading && (
          <div className="fixed top-4 right-4 bg-white rounded-lg shadow-lg p-4 z-50 border">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm font-medium text-gray-900">{t('uploading')}</span>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0">
              <h2 className="text-lg font-medium text-gray-900">{t('articles', { count: articles.length })}</h2>
              <button
          ...
          <div className="divide-y divide-gray-200">
            {articles.length === 0 ? (
              <div className="px-6 py-12 text-center">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">{t('noArticles')}</h3>
                <p className="mt-1 text-sm text-gray-500">{t('startAdding')}</p>
                {ARTICULOS_GAS_URL && (
                  <button
                    onClick={() => setShowAddModal(true)}
                    className="mt-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
                    aria-label={t('addFirstArticleAria')}
                  >
                    {t('addFirstArticle')}
                  </button>
                )}
                ...
                {articles.map((article, index) => (
                  <div key={index} className="hover:bg-gray-50">
                    <div
                      ...
                      aria-label={t('toggleArticleDetailsAria', { title: article['Título'] })}
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 text-sm text-gray-500 truncate" title={article['Autor(es)']}>
                          {article['Autor(es)'] || t('unknownAuthor')}
                        </p>
                      </div>
                    ...
                    {expanded[article['Número de artículo']] && (
                      <div className="px-6 pb-4 bg-gray-50">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div>
                            <p className="text-gray-900 font-medium">{t('abstract')}</p>
                            <p className="mt-1 text-gray-600">{article['Resumen'] || t('notAvailable')}</p>
                          </div>
                          <div>
                            <p className="text-gray-900 font-medium">{t('abstractEnglish')}</p>
                            <p className="mt-1 text-gray-600">{article['Abstract'] || t('notAvailable')}</p>
                          </div>
                          <div className="md:col-span-2">
                            <p className="text-gray-900 font-medium">{t('details')}</p>
                            <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                              <div>
                                <p className="text-gray-500">{t('date')}</p>
                                <p className="font-medium">{article['Fecha'] || t('na')}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">{t('volumeNumber')}</p>
                                <p className="font-medium">{`${article['Volumen'] || t('na')}/${article['Número'] || t('na')}`}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">{t('pages')}</p>
                                <p className="font-medium">{`${article['Primera página'] || ''}-${article['Última página'] || ''}`}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">{t('areas')}</p>
                                <p className="font-medium">{article.areas.length ? article.areas.join(', ') : t('na')}</p>
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            {showAddModal && (
              ...
              >
                ...
                >
                  <div className="mt-3">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">{t('addNewArticle')}</h3>
                    <form
                      className="space-y-4"
                      onSubmit={(e) => {
                        e.preventDefault();
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                          required
                          aria-label={t('authorsAria')}
                        />
                        <p className="mt-1 text-xs text-gray-500">{t('authorsHelp')}</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t('volumeLabel')}</label>
                          <input
                            name="volumen"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t('pagesLabel')}</label>
                          <input
                            name="primeraPagina"
                            placeholder={t('pageStartPlaceholder')}
                            value={formData.primeraPagina}
                            onChange={handleInputChange}
                            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                            aria-label={t('pageStartAria')}
                          />
                          <input
                            name="ultimaPagina"
                            placeholder={t('pageEndPlaceholder')}
                            ...
                            aria-label={t('pageEndAria')}
                          />
                        </div>
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('areaLabel')}</label>
                        <input
                          name="areaTematica"
                          value={formData.areaTematica}
                          onChange={handleInputChange}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                          aria-label={t('areaAria')}
                        />
                        <p className="mt-1 text-xs text-gray-500">{t('areaHelp')}</p>
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('keywordsLabel')}</label>
                        <input
                          name="palabrasClave"
                          value={formData.palabrasClave}
                          onChange={handleInputChange}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                          aria-label={t('keywordsAria')}
                        />
                        <p className="mt-1 text-xs text-gray-500">{t('keywordsHelp')}</p>
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('abstractLabel')}</label>
                        <textarea
                          name="resumen"
                          ...
                          aria-label={t('abstractAria')}
                        />
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('abstractEnglishLabel')}</label>
                        <textarea
                          name="abstract"
                          ...
                          rows="3"
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 resize-none"
                          aria-label={t('abstractEnglishAria')}
                        />
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('pdfFileLabel')}</label>
                        <input
                          type="file"
                          accept=".pdf"
                          ...
                          aria-label={t('pdfFileAria')}
                        />
                        <p className="mt-1 text-xs text-gray-500">{t('pdfFileHelp')}</p>
                      </div>
                      ...
                      <div className="pt-4 border-t border-gray-200 flex justify-end space-x-3">
                        ...
                        <button
                          ...
                          aria-label={t('addArticleSubmitAria')}
                        >
                          {uploading ? t('processing') : t('addArticleSubmit')}
                        </button>
                      </div>
                    </form>
                  </div>
                ...
                >
                  <div className="mt-3">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">
                      {t('editArticle', { numero: editingArticle['Número de artículo'] })}
                    </h3>
                    <form
                      className="space-y-4"
                      onSubmit={(e) => {
                    ...
                    >
                      ...
                      <div className="pt-4 border-t border-gray-200 flex justify-end space-x-3">
                        ...
                        <button
                          ...
                          disabled={!formData.titulo.trim() || !formData.autores.trim() || uploading}
                          className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                          aria-label={t('updateArticleSubmitAria')}
                        >
                          {uploading ? t('processing') : t('updateArticleSubmit')}
                        </button>
                      </div>
                    </form>
                  </div>
