    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-6 mt-4 pt-2" onClick={(e) => e.stopPropagation()}>
          {pdfUrl && (
  <a href={`/viewer.html?url=${encodeURIComponent(pdfUrl)}&doi=${article.doi || ''}&title=${encodeURIComponent(article.titulo || '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm font-semibold text-[#007398] hover:text-[#005a77] transition-colors">
