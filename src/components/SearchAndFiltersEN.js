        ...
        <div className="flex flex-col gap-3">
          ...
          <div className={`${showFilters ? 'grid' : 'hidden'} md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 pt-2 md:pt-0`}>
            ...
            <div className="lg:col-span-3">
              <label className="block md:hidden text-[9px] font-bold text-gray-400 uppercase mb-1">Volume</label>
              <select
                value={selectedVolume}
                onChange={(e) => setSelectedVolume(e.target.value)}
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-sm text-sm text-gray-600 outline-none"
              >
                <option value="">Volumes: All</option>
                {volumesList.map((vol) => <option key={vol} value={vol}>Volume {vol}</option>)}
              </select>
            </div>

            {/* Selector de Números */}
            <div className="lg:col-span-3">
              <label className="block md:hidden text-[9px] font-bold text-gray-400 uppercase mb-1">Issue</label>
              <select
              ...
              >
                ...
                {numbersList.map((num) => <option key={num} value={num}>Issue {num}</option>)}
              </select>
            </div>

            {/* Selector de Áreas */}
            <div className="lg:col-span-4">
              <label className="block md:hidden text-[9px] font-bold text-gray-400 uppercase mb-1">Discipline</label>
              <select
                value={selectedArea}
