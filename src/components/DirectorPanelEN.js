...
export default function DirectorPanelEN({ user }) {
  ...
  useEffect(() => {
    if (showAddModal) {
      const savedDraft = localStorage.getItem('draftNewArticleEN');
      if (savedDraft) {
  ...
  useEffect(() => {
    if (showEditModal && editingArticle) {
      const key = `draftEditArticleEN_${editingArticle.id}`;
  ...
  useEffect(() => {
    if (showAddModal) {
      localStorage.setItem('draftNewArticleEN', JSON.stringify(formData));
  ...
  if (!hasAccess) {
    return <div>Access denied. General Director only.</div>;
  }

  if (loading || volumeLoading) return <div>Loading...</div>;
  ...
  const handleSubmit = async (e, isVolume = false) => {
    ...
    try {
      ...
      if (!isVolume) {
        if (!isEdit) {
          localStorage.removeItem('draftNewArticleEN');
        } else {
          localStorage.removeItem(`draftEditArticleEN_${editing.id}`);
        }
      }
      closeFunc();
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-8">
          <div className="px-6 py-5 border-b border-gray-200">
            <h1 className="text-2xl font-bold text-gray-900">Director's Panel</h1>
          </div>
          <div className="p-6">
            <button
            ...
            {articles.length === 0 ? (
              ...
              <div className="max-h-96 overflow-y-auto">
                {articles.map((article) => {
                  const articleNumber = article.numeroArticulo || article.id.slice(0, 5);
                  return (
                        ...
                        <div className="px-6 pb-4 bg-gray-50">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <div className="mt-1 text-gray-600 whitespace-pre-line">{article.resumen || 'Not available'}</div>
                            </div>
                            <div>
                              <p className="text-gray-900 font-medium">Abstract (English)</p>
                              <div className="mt-1 text-gray-600 whitespace-pre-line">{article.abstract || 'Not available'}</div>
                            </div>
                            <div>
                              <p className="text-gray-900 font-medium">Keywords (Spanish)</p>
                              <p className="mt-1 text-gray-600">{article.palabras_clave?.join(', ') || 'Not available'}</p>
                            </div>
                            <div>
                              <p className="text-gray-900 font-medium">Keywords (English)</p>
                              <p className="mt-1 text-gray-600">{article.keywords_english?.join(', ') || 'Not available'}</p>
                            </div>
                            <div className="md:col-span-2">
                              ...
                              <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                  <p className="text-gray-500">Vol/No</p>
                                ...
                                <div>
                                  <p className="text-gray-500">Type (Spanish)</p>
                                  <p className="font-medium">{article.tipo || 'N/A'}</p>
                                </div>
                                <div>
                                  <p className="text-gray-500">Type (English)</p>
                                ...
                                <div>
                                  <p className="text-gray-500">Accepted Date</p>
              ...
              >
                <div className="mt-3">
                  ...
                  <form className="space-y-4" onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Author(s) * (separate with ;)</label>
                      <input
                        name="autores"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Keywords (Spanish, separate with ;)
                      </label>
                      <input
                        name="palabras_clave"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Keywords (English, separate with ;)
                      </label>
                      <input
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Type (Spanish)</label>
                      <input
                        name="tipo"
                        value={formData.tipo}
                        onChange={handleInputChange}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Type (English)</label>
                      <input
                        name="type"
                        value={formData.type}
                        onChange={handleInputChange}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Conflict of Interest (Spanish, optional)</label>
                      <textarea
                        name="conflicts"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Conflict of Interest (English, optional)</label>
                      <textarea
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Funding (Spanish, optional)</label>
                      <textarea
                        name="funding"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Funding (English, optional)</label>
                      <textarea
                        name="fundingEnglish"
                    ...
                    <div>
                      ...
                      />
                      <p className="mt-1 text-xs text-gray-500">PDF will be uploaded automatically</p>
                    </div>
            ...
            >
              ...
              >
                <div className="mt-3">
                  ...
                  <form className="space-y-4" onSubmit={(e) => handleSubmit(e)}>
                    {/* Same form fields as Add Modal, but with Edit button text */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Title *</label>
                      <input
                        name="titulo"
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden mt-8">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 text-sm text-gray-500 truncate" title={volume.volumen}>
                          Volume {volume.volumen}, Issue {volume.numero}
                        </p>
                      </div>
                      <div className="ml-4 flex-shrink-0 flex items-center space-x-2">
                        <span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                    ...
                    {volumeExpanded[volume.id] && (
                      <div className="px-6 pb-4 bg-gray-50">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          {volume.editorial && (
                            <div>
                              <p className="text-gray-900 font-medium">Editorial Note (Spanish)</p>
                              <p className="mt-1 text-gray-600">{volume.editorial || 'Not available'}</p>
                            </div>
                          )}
                          {volume.englishEditorial && (
                            <div>
                              <p className="text-gray-900 font-medium">Editorial Note (English)</p>
                              <p className="mt-1 text-gray-600">{volume.englishEditorial || 'Not available'}</p>
                            </div>
                          )}
                          <div className="md:col-span-2">
                            <p className="text-gray-900 font-medium">Details</p>
          ...
          {showAddVolumeModal && (
            ...
            >
              ...
              >
                <div className="mt-3">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Add New Volume</h3>
                  <form className="space-y-4" onSubmit={handleVolumeSubmit}>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Title (auto-generated if empty)</label>
                      <input
                        name="titulo"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">ISSN (optional)</label>
                      <input
                        name="issn"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Editorial Note (Spanish, optional)</label>
                      <textarea
                        name="editorial"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Editorial Note (English, optional)</label>
                      <textarea
                    ...
                    <div className="pt-4 border-t border-gray-200 flex justify-end space-x-3">
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {volumeUploading ? 'Processing...' : 'Add Volume'}
                      </button>
                    </div>
                  </form>
                </div>
          ...
          {showEditVolumeModal && editingVolume && (
            ...
            >
              ...
              >
                <div className="mt-3">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Edit Volume</h3>
                  <form className="space-y-4" onSubmit={handleVolumeSubmit}>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Volume *</label>
                      <input
                    ...
                    <div className="pt-4 border-t border-gray-200 flex justify-end space-x-3">
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {volumeUploading ? 'Updating...' : 'Update Volume'}
                      </button>
                    </div>
                  </form>
                </div>
