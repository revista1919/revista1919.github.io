  ...
  <div className="border border-slate-200 rounded-sm bg-white shadow-sm mb-8 overflow-hidden">
    ...
    <div className="divide-y divide-slate-100">
      {recommendations.map((reviewer) => {
        ...
        // Datos reales del engine
        const totalReviewsCompleted = reviewer.stats?.totalReviewsCompleted || 0;
        const isNewReviewer = totalReviewsCompleted < 3;
        const diversityPercent = Math.round((reviewer.scores?.diversity || 0.50) * 100);
        const explorationPercent = Math.round((reviewer.scores?.exploration || 0) * 100);
        ...
        return (
          ...
          >
            ...
            <div className="flex md:w-4/12 gap-4 items-start">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  <span className="inline-block text-[10px] px-2 py-0.5 bg-slate-100 text-slate-600 uppercase tracking-wider rounded-sm">
                    {reviewer.tierLabel}
                  </span>
                  {isNewReviewer && (
                    <span className="inline-block text-[9px] px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 uppercase tracking-wider rounded-sm">
                      {isSpanish ? 'Nuevo' : 'New'}
                    </span>
                  )}
                </div>
            ...
            <div className="md:w-3/12 flex flex-col justify-center border-t md:border-t-0 md:border-l border-slate-200 pt-4 md:pt-0 md:pl-6">
              ...
              {/* Desglose de scores reales del engine */}
              {reviewer.scores && (
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {Object.entries(reviewer.scores).map(([key, value]) => (
                    <span key={key} className="text-[9px] bg-slate-50 px-1.5 py-0.5 text-slate-500 uppercase tracking-wider rounded-sm border border-slate-200">
                      {key === 'expertise' && (isSpanish ? 'Exp.' : 'Exp.')}
                      {key === 'performance' && (isSpanish ? 'Rend.' : 'Perf.')}
                      {key === 'availability' && (isSpanish ? 'Disp.' : 'Avail.')}
                      {key === 'diversity' && (isSpanish ? 'Div.' : 'Div.')}
                      {key === 'exploration' && (isSpanish ? 'Expl.' : 'Expl.')}
                      {key === 'response' && (isSpanish ? 'Resp.' : 'Resp.')}
              ...
              <div className="flex items-center justify-between text-[10px]">
                ...
                <div className="bg-white border border-slate-200 px-2 py-1 rounded-sm shadow-sm flex flex-col items-center">
                  ...
                  <span className="font-mono font-bold text-[#002B49]">{totalReviewsCompleted}</span>
                </div>
              </div>
            </div>
            ...
            <div className="md:w-5/12 flex flex-col justify-center border-t md:border-t-0 md:border-l border-slate-200 pt-4 md:pt-0 md:pl-6">
              ...
              <ul className="space-y-2">
                ...
                {/* Diversidad real: solo si el score >= 70% */}
                {diversityPercent >= 70 && (
                  <li className="flex items-start gap-2 text-[11px] text-[#007398] leading-tight font-sans font-medium uppercase tracking-wider mt-2">
                    ...
                    {isSpanish 
                      ? `Institución no repetida en el panel (diversidad: ${diversityPercent}%)`
                      : `Institution not duplicated in panel (diversity: ${diversityPercent}%)`}
                  </li>
                )}
                
                {/* Exploración real: solo si es nuevo y el explorationScore > 10% */}
                {isNewReviewer && explorationPercent > 10 && (
                  <li className="flex items-start gap-2 text-[11px] text-purple-700 leading-tight font-sans font-medium uppercase tracking-wider mt-1">
                    <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                    </svg>
                    {isSpanish 
                      ? `Revisor con pocas evaluaciones (${totalReviewsCompleted} completadas). Exploration score: ${explorationPercent}%`
                      : `Reviewer with few evaluations (${totalReviewsCompleted} completed). Exploration score: ${explorationPercent}%`}
                  </li>
                )}
              </ul>
            </div>
