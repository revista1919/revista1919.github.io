...
export default function LoginSection({ onLogin }) {
  ...
  useEffect(() => {
    if (!auth) {
      ...
      setMessage('❌ Error en la configuración. Contacta al equipo.');
      return;
    }
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (!csvUser) {
          setMessage('❌ Este correo no está autorizado. ¿Eres autor o parte del equipo? Si sí, contáctanos por email.');
      ...
      try {
        ...
        Papa.parse(csvText, {
          ...
          complete: ({ data }) => {
            ...
            setUsers(validUsers);
            console.log(`✅ ${validUsers.length} usuarios autorizados cargados desde el CSV`);
            setIsLoading(false);
          },
          error: (err) => {
    ...
    try {
      ...
      switch (error.code) {
        case 'auth/email-already-in-use':
          errorMessage = 'Este correo ya está registrado. Intenta iniciar sesión o usa "Olvidé mi contraseña".';
          break;
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setMessage('✅ Revisa tu correo (incluyendo spam) para recuperar tu contraseña. Puede tardar unos minutos.');
    } catch (error) {
      console.error('Error forgot password:', error.code, error.message);
      ...
      switch (error.code) {
        case 'auth/invalid-email':
          ...
          break;
        case 'auth/user-not-found':
          errorMessage = 'No hay una cuenta registrada con este correo. Primero crea tu contraseña.';
          break;
        case 'auth/too-many-requests':
          errorMessage = 'Demasiados intentos. Espera unos 10–15 minutos.';
          break;
  ...
  // Pantalla inicial mientras carga CSV
