...
export default function TaskSection({ user }) {
  ...
  const fetchWithRetry = async (url, options, retries = MAX_RETRIES) => {
    try {
      ...
      if (err.name === 'AbortError') {
        throw new Error('La solicitud ha superado el tiempo máximo');
      }
      if (retries > 0) {
        console.warn(`Reintento ${MAX_RETRIES - retries + 1}/${MAX_RETRIES}: ${err.message}`);
        await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
        return fetchWithRetry(url, options, retries - 1);
      }
      throw new Error(`La solicitud falló después de ${MAX_RETRIES} reintentos: ${err.message}`);
    }
  };
  ...
  const fetchUsers = async () => {
    try {
      ...
      setError('No se puede leer el cuerpo de la respuesta en modo no-cors');
    } catch (err) {
      ...
      return new Promise((resolve, reject) => {
        reader.onload = async (e) => {
          ...
          try {
            ...
            reject(new Error('No se puede leer el cuerpo de la respuesta en modo no-cors'));
          } catch (err) {
            reject(err);
          }
        };
  ...
  const encodeBody = useCallback((html, editorRef = null) => {
    try {
      ...
      console.error('Error al codificar contenido:', err);
      try {
        const uriEncoded = encodeURIComponent(html);
        const base64 = btoa(uriEncoded);
        return base64;
      } catch (fallbackErr) {
        console.error('Error en codificación de respaldo:', fallbackErr);
  ...
  const handleAssignTask = async () => {
    ...
    console.log('HTML original de la tarea:', taskContent.substring(0, 200));
    const encodedTask = encodeBody(taskContent, taskEditorRef);
    console.log('Longitud de la tarea codificada:', encodedTask.length);
  ...
  const handleCompleteTask = async (task) => {
    ...
    console.log(`HTML original del comentario para la tarea ${task.rowIndex}:`, comment.substring(0, 200));
    const commentEditor = commentEditorsRef.current[task.rowIndex];
    const encodedComment = encodeBody(comment, commentEditor);
    console.log(`Longitud del comentario codificado para la tarea ${task.rowIndex}:`, encodedComment.length);
  ...
  const debouncedHandleCommentChange = useCallback(
    debounce((rowIndex, value) => {
      console.log(`Cambio en comentario para fila ${rowIndex}:`, value.substring(0, 100));
  ...
  const debouncedSetTaskContent = useCallback(
    debounce((value) => {
      console.log('Cambio en contenido de tarea:', value.substring(0, 100));
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col relative modal-content">
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4">
              ...
              <div className="text-xs text-gray-500">
                💡 Puedes usar <strong>formato enriquecido</strong>, agregar <strong>imágenes</strong> y <strong>enlaces</strong>. 
