...
export default function MailsTeam() {
  ...
  const sendEmail = async () => {
    ...
    const formattedBody = `
      ...
      <body>
        <div class="wrapper">
          <div class="container">
            <div class="header">
              <span class="journal-name">Revista Nacional de las Ciencias para Estudiantes</span>
            ...
            <div class="content">
              ...
              <div class="signature-area">
                ...
                Director General<br>
                <small style="text-transform: uppercase; font-style: normal; font-family: sans-serif; font-size: 10px; color: #888;">Cuerpo Editorial</small></p>
              </div>
            </div>
            <div class="footer">
              <p>&copy; ${new Date().getFullYear()} RNCE | Acceso Abierto a la Ciencia</p>
            </div>
          </div>
        </div>
      </body>
    ...
    try {
      ...
      setStatus('✅ Comunicación enviada exitosamente');
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="bg-gray-50 border-b border-gray-100 px-8 py-4 flex justify-between items-center">
                <div>
                  <h3 className="text-lg font-serif text-gray-900">Nueva Comunicación Editorial</h3>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Gestión de Correspondencia Interna</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-5 space-y-6 border-r border-gray-100 pr-8">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Destinatarios Seleccionados</label>
                    <div className="flex items-center justify-between mb-4 bg-[#f0f7f9] p-3 border border-[#007398]/20">
                      <span className="text-xs font-serif italic text-gray-700">{selected.length} miembros del equipo</span>
                      <input
                      ...
                      {manualEmails.map((email, i) => (
                        <div key={`manual-${i}`} className="flex items-center gap-3 p-2 hover:bg-gray-50 border-b border-gray-50 transition-colors">
                          ...
                          <div className="overflow-hidden">
                            ...
                            <p className="text-[9px] text-[#007398] uppercase tracking-tighter truncate">Destinatario manual</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Añadir Correo Externo</label>
                    <div className="flex gap-2">
                      ...
                      />
                      <button onClick={addManualEmail} className="px-3 py-2 bg-gray-100 text-[10px] font-bold uppercase rounded-sm hover:bg-gray-200">Añadir</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Asunto del Comunicado</label>
                    <input
                      type="text" value={subject} onChange={e => setSubject(e.target.value)}
                      placeholder="Ej: Convocatoria para Revisión de Pares - Vol. 4"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Cuerpo del Mensaje</label>
              ...
              <div className="bg-gray-50 border-t border-gray-100 px-8 py-4 flex items-center justify-between">
                <span className={`text-[11px] font-bold uppercase tracking-wider ${status.includes('✅') ? 'text-green-600' : status.includes('❌') ? 'text-red-600' : 'text-gray-400'}`}>
                  {status || (sending ? 'Procesando envío...' : 'Listo para transmitir')}
                </span>
                <div className="flex gap-4">
                  <button onClick={closeModal} className="px-6 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-black">Descartar</button>
                  <button
                  ...
                  >
                    {sending ? 'Enviando...' : 'Emitir Comunicación'}
                  </button>
                </div>
              </div>
            </motion.div>
