...
exports.uploadScientificNews = onRequest(
  ...
  async (req, res) => {
    ...
    const requestId = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    console.log(`[${requestId}] 🚀 uploadScientificNews - Iniciando petición`);

    try {
      // Verificar dependencias
      if (!Octokit || !fetch || !GoogleGenAI) {
        await loadDependencies();
        if (!Octokit || !fetch) {
          return res.status(500).json({ error: "Servicios no disponibles" });
        }
      ...
      try {
        ...
        const allowedRoles = ["Director General", "Editor en Jefe", "Editor de Sección", "Periodista", "Autor"];
        
        if (!roles.some(role => allowedRoles.includes(role))) {
          return res.status(403).json({ 
            error: "Se requiere rol de periodista o editor para publicar noticias" 
          });
        }
        console.log(`[${requestId}] ✅ Rol verificado: ${roles.join(', ')}`);
      } catch (roleError) {
      ...
      if (!title_es && !title_en) {
        return res.status(400).json({ error: "Se requiere al menos un título (español o inglés)" });
      }
      if (!body_es && !body_en) {
        return res.status(400).json({ error: "Se requiere al menos un cuerpo (español o inglés)" });
      }
      if (!author) {
        return res.status(400).json({ error: "El autor es requerido" });
      }
      if (!area_id) {
        return res.status(400).json({ error: "El área de la noticia es requerida" });
      }

      console.log(`[${requestId}] 📝 Datos recibidos - Área: ${area_id}, Autor: ${author}`);

      // Procesar traducciones usando IA (solo si faltan)
      ...
      if (!finalBodyEn && finalBodyEs) {
        ...
        finalBodyEn = await translateHtmlFragment(bodyDecoded, "es", "en");
      }

      // Normalizar los cuerpos HTML
      ...
      if (photo && photo.startsWith('data:image')) {
        ...
        try {
          ...
          const form = new FormData();
          form.append("image", cleanBase64);
          form.append("name", `news-${slug}`);

          const imgbbUrl = new URL("https://api.imgbb.com/1/upload");
          imgbbUrl.searchParams.set("key", IMGBB_API_KEY.value());
          ...
          const imgbbResponse = await fetch(imgbbUrl.toString(), {
            ...
            body: form,
            headers: form.getHeaders(),
            agent: imgbbUrl.protocol === 'https:' && httpsAgent ? httpsAgent : httpAgent
          });
      ...
      const newsItem = {
        ...
        slug: slug,
        title: {
          es: sanitizeInput(finalTitleEs),
          en: sanitizeInput(finalTitleEn)
        },
        content: {
          es: bodyEsNormalized,
          en: bodyEnNormalized
        ...
        metadata: {
          ...
          version: "1.0",
          wordCount: {
            es: bodyEsNormalized.replace(/<[^>]*>/g, '').split(/\s+/).filter(Boolean).length,
            en: bodyEnNormalized.replace(/<[^>]*>/g, '').split(/\s+/).filter(Boolean).length
          }
        },
        seo: {
          title: sanitizeInput(finalTitleEs),
          description: bodyEsNormalized.replace(/<[^>]*>/g, '').substring(0, 160),
          keywords: [...(Array.isArray(tags) ? tags : []), area_id].join(', ')
        }
      };
      ...
      if (existingYearNews) {
        yearNews = existingYearNews.content.news || existingYearNews.content || [];
      }
      ...
      if (existingIndex) {
        indexData = existingIndex.content;
        indexData.last_updated = fechaIso;
      ...
      indexData.years[year] = {
        ...
        total_news: yearNews.length,
        last_updated: fechaIso,
        first_news_date: yearNews[yearNews.length - 1]?.metadata?.createdAt || fechaIso,
      ...
      try {
        await octokit.request("POST /repos/{owner}/{repo}/dispatches", {
          owner: "revista1919",
          repo: "revista1919.github.io",
          event_type: "rebuild-scientific-news",
          client_payload: {
            action: "add",
            newsId: newsId,
            slug: slug,
      ...
      return res.json({
        ...
        message: "Noticia científica publicada exitosamente"
      });
...
exports.submitArticle = onRequest(
  ...
  console.log(`📱 Teléfono de correspondencia: ${correspondingAuthorData.phone}`);
}
 ...
 for (const editor of editorEmails) {
  const authorsList = processedAuthors.map(a => {
    let authorInfo = `• ${a.firstName} ${a.lastName} (${a.email})`;
    
    if (a.isCorresponding) {
      authorInfo += ' 📧 [CORRESPONDENCIA]';
    }
    ...
    if (a.phone) {
      authorInfo += ` 📱 ${a.phone}`;  // ← Template literal simple
    }
  ...
  ? `<p><strong>📱 Teléfono de correspondencia:</strong> ${correspondingAuthorData.phone}</p>`
  : '';
